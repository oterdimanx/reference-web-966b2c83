    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className="h-9 w-9 chrome-button relative overflow-hidden">
          {(() => {
            const currentPreference = user && preferences ? preferences.theme_preference : theme;
            
            if (currentPreference === 'light') {
              return <Sun className="h-4 w-4 text-amber-500 transition-all duration-500" />;
            } else if (currentPreference === 'dark') {
              return <Moon className="h-4 w-4 text-indigo-400 transition-all duration-500" />;
            } else {
              return <Monitor className="h-4 w-4 text-teal-500 transition-all duration-500" />;
            }
          })()}
